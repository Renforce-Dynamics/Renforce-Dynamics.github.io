...
// Background mode: full-screen, autonomous, looping hero background
const urlParams = new URLSearchParams(window.location.search);
const isBackgroundMode = urlParams.get('bg') === '1' || urlParams.get('background') === '1';
...
export class MuJoCoDemo {
  ...
  async init() {
    ...
    if (this.isBackgroundMode && this.policyController) {
      this.policyController.highSpeedMode = true;
      this.policyController.autoForward = true;
    }

    // Start the render loop only after the model and assets are ready
    this.renderer.setAnimationLoop( this.render.bind(this) );
    if (this.isBackgroundMode) {
      this.backgroundRunStartTime = performance.now();
    }
  }
  ...
  async render(timeMS) {
    ...
    // Background mode: reset the run every 30 seconds to create a seamless loop
    if (this.isBackgroundMode && this.backgroundRunStartTime > 0 && !this.params.paused) {
      const elapsed = performance.now() - this.backgroundRunStartTime;
      if (elapsed >= 30000) {
        this.applySceneInitialState({ resetData: true, rebindCameras: false });
        this.backgroundRunStartTime = performance.now();
        return;
      }
    }
